...
void drawtext(const char *text, int length, int x,int y){
	...
	glGetDoublev(GL_PROJECTION_MATRIX, matrix);			//return the double value of a matrix parameter
	glLoadIdentity();									// replace the current matrix with the identity matrix
	glOrtho(0.0,600,0.0,600,-5,5);
	glRasterPos2i(x,y);									//specify the raster position for pixel operations
	
	for(int i=0; i<length;i++){
		glutBitmapCharacter(GLUT_BITMAP_HELVETICA_18,(int)text[i]);
	}
	glLoadMatrixd(matrix);								//replace the current matrix with the 'matrix' matrix
}
